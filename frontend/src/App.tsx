...
function AppContent() {
  ...
  const handleLoginSuccess = (userData: any, accessToken: string) => {
    ...
    fetch('https://medclinic-demo.onrender.com/users/me', { headers: { Authorization: `Bearer ${accessToken}` } })
  ...
  return (
    <div className="app-container modern-theme" onClick={() => { setIsMenuOpen(false); setIsMobileMenuOpen(false); setIsLangMenuOpen(false); }}> 
                  ...
                  <div className="user-nav-container">
                    ...
                    {isMenuOpen && (
                      <div className="user-dropdown-modern">
                        {user.role === 'DOCTOR' && <div onClick={() => handleNavigate('doctor-panel')} className="drop-item">👨‍⚕️ Panel</div>}
                        {['RECEPTIONIST', 'MANAGER', 'ADMIN'].includes(user.role) && <div onClick={() => handleNavigate('quick-search')} className="drop-item highlight">🔍 Wyszukaj</div>}
      ...
      <main className="app-main-content modern-bg">
        <Routes>
          <Route path="/" element={<Navigate to="/pl" replace />} />
          <Route path="/:lang" element={<HomePage lang={lang} clinicData={activeClinicData} allClinics={clinics} onBookClick={() => handleNavigate('list')} onClinicSelect={(id) => navigateUrl(`/${lang.toLowerCase()}/klinika/${id}`)} />} />
          <Route path="/:lang/klinika/:clinicId" element={<HomePage lang={lang} clinicData={activeClinicData} allClinics={clinics} onBookClick={() => handleNavigate('list')} onClinicSelect={(id) => navigateUrl(`/${lang.toLowerCase()}/klinika/${id}`)} />} />
          <Route path="/:lang/nasi-lekarze" element={<DoctorsDirectory lang={lang} doctors={clinicSpecificDoctors} onBookDirectly={(doc, svc) => { setPreselectedService(svc); setSelectedDoctor(doc); }} />} />
          <Route path="/:lang/umow-wizyte" element={<div className="standard-view-padding">{loading ? <div className="loader-box">Wczytywanie...</div> : <DoctorsList doctors={clinicSpecificDoctors} loading={loading} lang={lang} onBook={(doc) => setSelectedDoctor(doc)} />}</div>} />
          <Route path="/:lang/logowanie" element={<div className="auth-view-center"><LoginForm onLoginSuccess={handleLoginSuccess} /><p className="auth-switch">Nie masz konta? <span onClick={() => handleNavigate('register')}>Zarejestruj się</span></p></div>} />
          <Route path="/:lang/rejestracja" element={<div className="auth-view-center"><RegisterForm onSuccess={() => handleNavigate('login')} onSwitchToLogin={() => handleNavigate('login')} lang={lang} /></div>} />
          <Route path="/:lang/moj-profil" element={<div className="standard-view-padding"><UserProfile user={user} onUpdateUser={setUser} lang={lang} /></div>} />
          <Route path="/:lang/panel-lekarza" element={user?.role !== 'DOCTOR' ? <p className="error-notice">Brak uprawnień.</p> : <DoctorDashboard />} />
          <Route path="/:lang/radar-gabinetow" element={!['RECEPTIONIST', 'ADMIN', 'MANAGER'].includes(user?.role) ? <p className="error-notice">Brak uprawnień.</p> : <ReceptionPanel user={user} onBookVisit={handleBookForPatient} />} />
          <Route path="/:lang/grafik" element={!user ? <p className="error-notice">Zaloguj się.</p> : <SchedulePanel userRole={user.role} userId={user.id} />} />
          <Route path="/:lang/kartoteka" element={!['RECEPTIONIST', 'ADMIN', 'MANAGER'].includes(user?.role) ? <p className="error-notice">Brak uprawnień.</p> : <PatientsRegistry onBookVisit={handleBookForPatient} />} />
          <Route path="/:lang/ustawienia" element={user?.role !== 'ADMIN' ? <p className="error-notice">Brak uprawnień.</p> : <AdminSettings onRefreshData={fetchDoctorsAndClinics} />} />
          <Route path="/:lang/wyszukaj-termin" element={!['RECEPTIONIST', 'MANAGER', 'ADMIN'].includes(user?.role) ? <p className="error-notice">Brak uprawnień.</p> : <QuickSearch doctors={clinicSpecificDoctors} onBook={(doc, svc, date, time) => { setPreselectedService(svc); setPreselectedDate(date); setPreselectedTime(time); setSelectedDoctor(doc); }} />} />
          <Route path="/:lang/sprawdz-wizyte" element={<div className="standard-view-padding"><div className="guest-search-card"><h2>{t.checkGuestTitle}</h2><form onSubmit={handleGuestCheckSubmit} className="search-pill-form"><input type="text" placeholder={t.peselPlaceholder} value={guestPeselInput} onChange={(e) => setGuestPeselInput(e.target.value.replace(/\D/g, ''))} maxLength={11} /><button type="submit" className="pill-btn-primary">{t.searchBtn}</button></form>{activeGuestPesel && <MyAppointments lang={lang} guestPesel={activeGuestPesel} />}</div></div>} />
          <Route path="/:lang/moje-wizyty" element={<div className="standard-view-padding"><MyAppointments lang={lang} /></div>} />
        </Routes>
      </main>
        ...
        <div style={{ maxWidth: '1440px', margin: 'auto', width: '100%' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '40px', marginBottom: '50px' }}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <p style={{ fontSize: '0.95rem', lineHeight: 1.6, margin: 0 }}>Nowoczesna opieka medyczna i system Smart-Wait.</p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <p style={{ fontSize: '0.95rem', lineHeight: 1.6, margin: 0, fontWeight: 600 }}>
                {activeClinicData?.address || 'Warszawa'}<br/>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f1f5f9', paddingTop: '20px' }}>
            ...
            <button onClick={() => window.scrollTo(0, 0)} className="pill-btn-outline" style={{ padding: '8px 16px' }}>↑ Góra</button>
          </div>
        </div>
      </footer>
    </div>
